...
void FC::Start(){
  ...
  while(armed){
    ...
    if (Logger.enableserial && tCtr == 75){Logger.SerialSendFrame(); tCtr = 0;} // Send telemetry over serial when activated
  }
}
...
void FC::InputTransform(){
  ...
  if (rx_raw[0] >= PWM_CENTER+cfg.rx_deadzone){ // Stick Right
    rx_scaled[2] += map( rx_raw[0],  PWM_CENTER,  2000,  0,  cfg.max_angle )/10; // Reduced speed by dividing
  }
  else if (rx_raw[0] < PWM_CENTER-cfg.rx_deadzone){ // Stick Left
    rx_scaled[2] -= map( rx_raw[0],  PWM_CENTER,  1000,  0,  cfg.max_angle )/10; // Reduced speed by dividing
  }
}
